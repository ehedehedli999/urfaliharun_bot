...
Sen son derece dahi, üstün zekalı, derin bilgi sahibi ve kusursuz bir yapay zekasın.
...
Gereksiz uzatmadan doğrudan, dahi ve en net cevabı ver.
"""

TRANSLATION_SYSTEM_PROMPT = """
Sen profesyonel, birebir anlamı koruyan hassas bir tercümansın.
Görevin metni orijinal anlamına %100 sadık kalarak diğer dillere çevirmektir.

ÇOK SIKI KURAL VE YASAKLAR:
1. GİRDİ METNİNİN DİLİNİ ASLA TEKRAR ETME VEYA KENDİ DİLİNE ÇEVİRME!
   - Mesaj RUSÇA ise -> Yanıtta ASLA 🇷🇺 Rusça olmayacak!
   - Mesaj TÜRKÇE ise -> Yanıtta ASLA 🇹🇷 Türkçe olmayacak!
   - Mesaj ALMANCA ise -> Yanıtta ASLA 🇩🇪 Almanca olmayacak!

2. METNE ASLA KENDİ KAFANDAN YORUM, EKLEME VEYA UYDURMA ANLAM KATMA!
   - Orijinal cümlede ne yazıyorsa birebir, en doğru ve doğal karşılığını aktar.

3. KESİNLİKLE "Türkçe:", "Rusça:", "Almanca:" gibi başlıklar yazma. Sadece bayrak emojisi koy.

FORMAT ÖRNEKLERİ:
Girdi Rusça ise:
🇹🇷 [Doğrudan Türkçe Çevirisi]
🇩🇪 [Doğrudan Almanca Çevirisi]

Girdi Türkçe ise:
🇷🇺 [Doğrudan Rusça Çevirisi - Kiril Alfabesiyle]
🇩🇪 [Doğrudan Almanca Çevirisi]
"""
...
async def query_grok(prompt: str, system_prompt: str) -> str:
    try:
        ...
        data = {
            ...
            "temperature": 0.0, # Sıfır yaratıcılık: Tam sadık çeviri için
        }
...
Aşağıdaki görevi 3 dilde (Türkçe, Rusça, Almanca) bayraklı hazırla:
🇹🇷 [Türkçe Metin]
🇷🇺 [Rusça Metin - Kiril]
🇩🇪 [Almanca Metin]
"""

# --- KALAN TEMEL KOMUTLAR ---
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    p = "Hakkımda metnini hazırla: Ben Viyana Ai Yapay zeka botu Ehed Tarafından Tasarlandım en Üst düzey Ai Texnolojisiyle calisiyorum."
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Bot Etiketlendiyse VEYA Bota Cevap Verildiyse (Dahi Mod - Yazılan Dilde Yanıt)
        ...
        # Filtre: Çok kısa kelimeler ve linkler çevrilmez
        words = text.split()
        if len(words) < 2 or len(text) < 4 or "http" in text:
            return

        # Otomatik Çeviri
        active_prompt = f"{TRANSLATION_SYSTEM_PROMPT}\n"
        active_prompt += f"Aktif Çeviri Durumları: TR={LANG_STATUS['tr']}, RU={LANG_STATUS['ru']}, DE={LANG_STATUS['de']}. Kapalı olan dilde çıktı verme!"

        translated = await query_grok(text, active_prompt)
...
if __name__ == "__main__":
    ...
    print("Viyana AI (Sıfır Hata & Kusursuz Çeviri) Çalışıyor...")
